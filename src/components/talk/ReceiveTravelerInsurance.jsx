...
const ReceiveTravelerInsurance = () => {
  ...
  // 여행자 보험신청
  const seoulStation = () => {
    const thisChatData = chatBox.filter((item) => item.name === "KBChat6")
    dispatch(setSpliceChatBox(chatBox.indexOf(thisChatData[0])))
    dispatch(setChatBox(SettingChatData("KBChat7-1", chatCount)))
		setTimeout(function(){
			dispatch(setChatBox(SettingChatData("KBChat7-2", chatCount)))
		}, 1400)
  }
